...
export class SchedulingUtils {
  ...
  static validateSchedule(schedule: ScheduleOptions): ScheduleValidationResult {
    ...
    if (!schedule) {
      errors.push({ code: 'SCHEDULE_REQUIRED', message: 'Schedule is required', field: 'schedule', value: schedule })
    ...
    if (!hasValidSchedule) {
      errors.push({ code: 'NO_VALID_SCHEDULE', message: 'At least one valid schedule type (at, on, every) is required', field: 'schedule', value: schedule })
    }
  ...
  static validateAtSchedule(at: Date): ScheduleValidationResult {
    ...
    if (isNaN(date.getTime())) {
      errors.push({ code: 'INVALID_DATE', message: 'Invalid date format for "at" schedule', field: 'at', value: at })
    } else if (date <= new Date()) {
      errors.push({ code: 'PAST_DATE', message: 'Schedule date must be in the future', field: 'at', value: at })
    }
  ...
  static validateOnSchedule(on: DateComponents): ScheduleValidationResult {
    ...
    if (on.year !== undefined) {
      ...
      if (on.year < currentYear || on.year > currentYear + 100) {
        errors.push({ code: 'INVALID_YEAR', message: 'Year must be between current year and 100 years from now', field: 'on.year', value: on.year })
      }
    }

    if (on.month !== undefined) {
      if (on.month < 1 || on.month > 12) {
        errors.push({ code: 'INVALID_MONTH', message: 'Month must be between 1 and 12', field: 'on.month', value: on.month })
      }
    }

    if (on.day !== undefined) {
      if (on.day < 1 || on.day > 31) {
        errors.push({ code: 'INVALID_DAY', message: 'Day must be between 1 and 31', field: 'on.day', value: on.day })
      }
    }

    if (on.hour !== undefined) {
      if (on.hour < 0 || on.hour > 23) {
        errors.push({ code: 'INVALID_HOUR', message: 'Hour must be between 0 and 23', field: 'on.hour', value: on.hour })
      }
    }

    if (on.minute !== undefined) {
      if (on.minute < 0 || on.minute > 59) {
        errors.push({ code: 'INVALID_MINUTE', message: 'Minute must be between 0 and 59', field: 'on.minute', value: on.minute })
      }
    }

    if (on.second !== undefined) {
      if (on.second < 0 || on.second > 59) {
        errors.push({ code: 'INVALID_SECOND', message: 'Second must be between 0 and 59', field: 'on.second', value: on.second })
      }
    }

    if (on.weekday !== undefined) {
      const validWeekdays: WeekDay[] = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday']
      if (!validWeekdays.includes(on.weekday)) {
        errors.push({ code: 'INVALID_WEEKDAY', message: 'Invalid weekday value', field: 'on.weekday', value: on.weekday })
      }
    }
  ...
  static validateEverySchedule(every: RepeatOptions): ScheduleValidationResult {
    ...
    const validIntervals: RepeatOptions['interval'][] = ['minute', 'hour', 'day', 'week', 'month', 'year']
    if (!validIntervals.includes(every.interval)) {
      errors.push({ code: 'INVALID_INTERVAL', message: 'Invalid interval type', field: 'every.interval', value: every.interval })
    }

    if (every.count !== undefined && every.count < 1) {
      errors.push({ code: 'INVALID_COUNT', message: 'Count must be at least 1', field: 'every.count', value: every.count })
    }

    if (every.until !== undefined) {
      const untilDate = new Date(every.until)
      if (isNaN(untilDate.getTime())) {
        errors.push({ code: 'INVALID_UNTIL_DATE', message: 'Invalid until date format', field: 'every.until', value: every.until })
      } else if (untilDate <= new Date()) {
        errors.push({ code: 'PAST_UNTIL_DATE', message: 'Until date must be in the future', field: 'every.until', value: every.until })
      }
    }

    if (every.frequency !== undefined && every.frequency < 1) {
      errors.push({ code: 'INVALID_FREQUENCY', message: 'Frequency must be at least 1', field: 'every.frequency', value: every.frequency })
    }
  ...
  static getScheduleDescription(schedule: ScheduleOptions): string {
    ...
    if (schedule.on) {
      ...
      if (schedule.on.hour !== undefined) parts.push(`${schedule.on.hour}:${schedule.on.minute || 0}`)
